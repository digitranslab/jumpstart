...
5. Verify if JumpStart is running by checking by checking the status of the tasks in the created service.
...
### Creating default admin workspace and account
  ...
  1.  Create a new revision from your existing task definition.
      You can change task definition name to `jumpstart-seed-task`
  ...
  5.  Configure your run task with the `jumpstart-seed-task` for your cluster. It should look like:
