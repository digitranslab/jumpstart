...
:::info
You should manually set up a PostgreSQL database to be used by JumpStart. We recommend using **Cloud SQL** for this purpose.
Also for deploying JumpStart 3.0, Redis, Postgrest along with PostgreSQL are required.
:::
...
- **Google Cloud Run service**
  - **jumpstart-app (container - 1)**
  - **postgrest (container - 2)**
  - **redis (container - 3)**
- **Cloud SQL (for PostgreSQL)**
  - **for both (JUMPSTART_DB and PG_DB)** 
...
4. Under environmental variables, please add the below JumpStart application variables. 
  
  You can use these variables in the container 1: jumpstart-app
...
| `JUMPSTART_DB_USER`          | `postgres`                   |
| `JUMPSTART_DB_HOST`          | `<postgresql-instance-ip>`    |
| `JUMPSTART_DB_PASS`          | `<password>`                 |
| `PGRST_HOST`               | `localhost:3001`             |
| `PGRST_JWT_SECRET`         | `<generate using open ssl>`   |
| `REDIS_HOST`               | `localhost`                  |
| `REDIS_PORT`               | `6379`                       |
| `REDIS_USER`               | `default`                    |
| `JUMPSTART_HOST`             | `<Endpoint url>`             |
...
2. You can add the following environment variables in the **variables and secrets of postgrest container**, for the postgrest container to communicate to the **jumpstart-app**.

| **Environment Variable**   | **Value**                                                   |
|-----------------------------|------------------------------------------------------------|
| `PGRST_DB_PRE_CONFIG`      | `postgrest.pre_config`                                      |
| `PGRST_JWT_SECRET`         | `<generate using openssl>`                                  |
| `PGRST_DB_URI`             | `postgres://<user>:password@<jumpstart_db_host>/<jumpstart_db>` |
...
New LTS versions are released every 3-5 months with an end-of-life of 18 months. To check the latest LTS version, visit the [JumpStart Docker Hub](https://hub.docker.com/r/digitranslab/jumpstart/tags) page. The LTS tags follow a naming convention with the prefix `LTS-` followed by the version number, for example `digitranslab/jumpstart:ee-lts-latest`.
