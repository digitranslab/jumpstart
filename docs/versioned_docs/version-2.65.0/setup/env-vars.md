...
### Enable JumpStart Database ( optional )
...
Use `ENABLE_JUMPSTART_DB` to enable/disable the feature that allows users to work with inbuilt data store to build apps with. In order to set it up, [follow the instructions here](/docs/jumpstart-database#enabling-the-jumpstart-database-for-your-instance).
...
For Enterprise Edition, you must configure SMTP settings through the user interface (UI) in the JumpStart Settings. For more information, see [SMTP Configuration](../org-management/smtp-configuration.md).
