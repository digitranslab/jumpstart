...
1. If you are using dev database within JumpStart deployment, upgrade it to managed database. You could also add a separate database, if you intent use a differenet database, please refer the [environment variables](/docs/setup/env-vars/#enable-jumpstart-database--optional--feature-enable--optional-) for additional env variables. 
...
5. Update your existing JumpStart application deployment with [environment variables](/docs/setup/env-vars/#enable-jumpstart-database--optional--feature-enable--optional-) required for PostgREST. 
