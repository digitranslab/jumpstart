...
# Check if PG_PASS and JUMPSTART_DB_PASS are present or empty
if [[ -z "$PG_PASS" ]] && [[ -z "$JUMPSTART_DB_PASS" ]]; then
  ...
  awk -v pass="$PASSWORD" '
    BEGIN { FS=OFS="=" }
    /^(PG_PASS|JUMPSTART_DB_PASS)=/ { $2=pass; found=1 }
    1
    END { if (!found) print "PG_PASS="pass ORS "JUMPSTART_DB_PASS="pass }
  ' .env > temp.env && mv temp.env .env
...
if [[ -z "$PGRST_DB_URI" ]]; then
  ...
  PGRST_DB_URI="postgres://postgres:$PASSWORD@postgresql/jumpstart_db"
